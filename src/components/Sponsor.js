...
function Sponsor(props) {
    function click(){
        // props.moreDetails(props.Sponsor)
    }
  return (
    <div className="xl:w-1/4 lg:w-1/2 md:w-full px-8 py-6 border-l-2 border-slate-400 border-opacity-60">
        <h2 className="text-lg sm:text-xl text-slate-50 font-medium title-font mb-2">{props.sponsor.name}</h2>
        <time className="mb-1 text-sm font-normal leading-none text-slate-400">{props.sponsor.date} : {props.sponsor.time}</time>
        <p className="leading-relaxed text-slate-400 mb-4">{props.sponsor.headline}</p>
